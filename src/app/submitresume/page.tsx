...
export default function SubmitResumePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 mt-20 bg-[#05041c] rounded-xl shadow-2xl shadow-[#347188]/20 border border-[#347188]/30">
      <h1 className="text-3xl font-bold mb-4 text-blue-200 drop-shadow-lg">
        Submit Resumes for: <span className="text-blue-400">{job.position}</span>
      </h1>
      <p className="mb-6 text-blue-300">Openings: {job.openings}</p>

      <div className="mb-8">
        <label className="block text-base font-semibold text-blue-300 mb-2">
        ...
        <input
          ...
          className="block w-full text-sm text-gray-100 bg-slate-800 border border-slate-600 rounded-lg p-3 focus:ring-2 focus:ring-[#347188] focus:border-[#347188] transition-all duration-300 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#347188] file:text-white hover:file:bg-green-700"
        />
      </div>

      {resumes.length > 0 && (
        <div className="mb-8">
          <h2 className="text-lg font-semibold mb-3 text-blue-200">Selected Resumes</h2>
          <ul className="space-y-3">
            {resumes.map((file, index) => (
              <li
                key={index}
                className="flex items-center justify-between p-3 border border-blue-700 rounded-lg bg-slate-900"
              >
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-100">{file.name}</p>
                  {uploadProgress[index] > 0 && uploadProgress[index] < 100 && (
                    <div className="w-full bg-blue-900 rounded-full h-2.5 mt-1">
                      <div
                ...
                <button
                  ...
                  className="text-red-400 hover:text-red-600 ml-4 font-semibold"
                >
                  Remove
                </button>
              </li>
      ...
      <div className="flex space-x-4">
        ...
        <button
          ...
          className="px-8 py-3 bg-slate-800 text-blue-200 font-bold rounded-lg hover:bg-red-600 transition-all duration-300 shadow-lg shadow-blue-900/20 hover:shadow-xl hover:shadow-blue-900/40"
        >
          Cancel
        </button>
        ...
        {resumes.length > 0 && uploadComplete && (
          <>
            <button
              ...
              className="px-8 py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 transition-all duration-300 shadow-lg shadow-green-600/20 hover:shadow-xl hover:shadow-green-600/40 ml-auto disabled:bg-green-300 disabled:cursor-not-allowed"
            >
